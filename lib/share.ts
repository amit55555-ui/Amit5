...
export const APP_URL = 'https://metzion.co.il';
...
function productMessage(p: Product): string {
  ...
  return `😍 מצאתי מציאה מהממת ב"מציאון": ${p.name}${price}\n${p.link}\n\n🛍️ עוד אלפי מציאות: ${APP_URL}`;
}
...
export async function shareList(products: Product[]): Promise<'native' | 'whatsapp'> {
  ...
  const text = `🛍️ הנה המציאות שאהבתי ב"מציאון":\n\n${lines.join('\n\n')}\n\n✨ גלה עוד: ${APP_URL}`;
  if (typeof navigator !== 'undefined' && navigator.share) {
    try {
